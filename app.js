...
(function () {
    ...
    function setupEvents() {
        ...
        // Sync Modal
        $('btn-cloud-sync').addEventListener('click', () => openModal('modal-sync'));

        $('btn-sync-create').addEventListener('click', async () => {
            try {
                // Fetch a new bucket from kvdb
                $('btn-sync-create').textContent = "Generando...";
                $('btn-sync-create').disabled = true;
                
                // Generamos un ID aleatorio local para evitar requerir email ahora
                const newCloudId = 'nut_' + Math.random().toString(36).substring(2, 10) + Math.random().toString(36).substring(2, 10);
                
                cloudId = newCloudId;
                localStorage.setItem(CLOUD_KEY, cloudId);
                
                // Force sync current data to the new cloud ID
                await fetch(`https://kvdb.io/${cloudId}/nutcontrol`, {
                    method: 'PUT',
                    body: JSON.stringify(state)
                });

                $('sync-code-display').style.display = 'block';
                $('sync-code-value').textContent = cloudId;
                
                updateStorageUI();
                showToast('☁️ Base de datos creada. Ahora todos tus cambios se suben a la nube.');
            } catch (err) {
                console.error("Sync Create Error:", err);
                showToast('Error al conectar con la nube.', true);
            } finally {
                $('btn-sync-create').textContent = "☁️ Generar Nuevo Código de Nube";
                $('btn-sync-create').disabled = false;
            }
        });

        $('btn-sync-open').addEventListener('click', async () => {
            const inputCode = $('sync-code-input').value.trim();
            if (!inputCode) {
                showToast('Ingresá el Código Secreto', true);
                return;
            }
            
            try {
                $('btn-sync-open').textContent = "Vinculando...";
                $('btn-sync-open').disabled = true;

                const response = await fetch(`https://kvdb.io/${inputCode}/nutcontrol`);
                if (!response.ok) {
                    throw new Error('Código inválido o base de datos vacía');
                }
                ...
                if (imported.materias && imported.productos) {
                    ...
                    showToast('☁️ Dispositivo vinculado exitosamente a la nube.');
                } else {
                    showToast('Los datos en la nube están corruptos', true);
                }
            } catch (err) {
                console.error("Sync Open Error:", err);
                showToast('Error: ' + err.message, true);
            } finally {
                $('btn-sync-open').textContent = "Vincular";
                $('btn-sync-open').disabled = false;
            }
        });
